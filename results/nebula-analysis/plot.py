...
dataset. The chart file numbers match the figure numbers in the main
body of docs/nebula-autonat-analysis.md:

  Figure 1: 01_dialable_over_time.png  ← 05_dialable_over_time.csv
  Figure 2: 02_clients.png              ← 01_clients.csv
  Figure 3: 03_autonat_protocols.png    ← 02_autonat_protocols.csv
  Figure 4: 04_server_mode.png          ← 03_server_mode.csv
  Figure 5: 05_dialability_vs_public.png ← 08_dialability_vs_public.csv

  (appendix)  06_oscillation_refined_appendix.png  ← 06_oscillation_refined.csv

Note: CSV file names still reflect the original query order — only the
chart file names were renumbered to match figure numbers in the doc.
Earlier charts 04_oscillation.csv and 07_joint_state.csv are no longer
plotted (superseded by the refined charts).
...
def chart_clients():
    ...
    fig.tight_layout()
    fig.savefig(ROOT / "02_clients.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print("wrote 02_clients.png")
...
def chart_autonat_protocols():
    ...
    fig.tight_layout()
    fig.savefig(ROOT / "03_autonat_protocols.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print("wrote 03_autonat_protocols.png")
...
def chart_server_mode():
    ...
    fig.tight_layout()
    fig.savefig(ROOT / "04_server_mode.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print("wrote 04_server_mode.png")
...
def chart_dialable_over_time():
    ...
    fig.tight_layout()
    fig.savefig(ROOT / "01_dialable_over_time.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print("wrote 01_dialable_over_time.png")
...
def chart_oscillation_refined():
    ...
    fig.tight_layout()
    fig.savefig(ROOT / "06_oscillation_refined_appendix.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print("wrote 06_oscillation_refined_appendix.png")


# ----------------------------------------------------------------------
# Chart 6: 2D heatmap — dialability fraction × Public-state fraction
# (was previously chart 8; chart 4 and chart 7 removed as redundant)
# ----------------------------------------------------------------------
def chart_dialability_vs_public():
    ...
    fig.tight_layout()
    fig.savefig(ROOT / "05_dialability_vs_public.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
    print("wrote 05_dialability_vs_public.png")

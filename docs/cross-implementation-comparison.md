...
|-------|-----------|-------------|-----------|
| **Inconsistent global vs per-address reachability** (F1) | **Yes** — DHT/AutoRelay consume v1's `EvtLocalReachabilityChanged` and are blind to v2; v1 oscillates in 60% of runs with unreliable peers and the unstable global flag overrides v2's stable per-address result | Not affected today — DHT consumes `ExternalAddrConfirmed` (v2-fed), but no documented reduction if both v1 and v2 ran | Not affected today — DHT consumes `self:peer:update` (v2-fed), but Helia uses v1 only and no documented reduction either |
